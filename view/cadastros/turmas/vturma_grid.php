...
DatabaseGrid.prototype.fetchGrid = function(table)  {
    ...
    //if(table == "TURMA")
        ...
        this.editableGrid.loadJSON("<?php echo ROOT_URL."control/cadastros/turmas/loaddata_turma_grid.php" ?>");
};
...
DatabaseGrid.prototype.abrirJanela = function(id) {
    window.location = '<?php echo ROOT_URL."control/cadastros/turmas/edit_turma.php?t=" ?>'+id;
}
...
DatabaseGrid.prototype.addRow = function(id,table) 
{
  ...
  var self = this;
        ...
        $.ajax({
		url: 'gravar_turma.php',
		type: 'POST',
		dataType: "html",
		data: {
			tablename : self.editableGrid.name,
			serie:  $("#campo-serie").val(),
			letra:  $("#campo-letra").val(),
                        ano:  $("#campo-ano").val(),
                        redirecionar:  0,
                        prof:  <?php echo $_SESSION['USUARIO_CDG'] ?>
		},
		success: function (response){                   
			if (response == "" ) {
                            // hide form
