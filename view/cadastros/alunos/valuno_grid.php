...
{      
    //editableGrid.getColumnName(0)
    //alert(editableGrid.getColumnName(0) +', rowindex = '+ rowIndex+','+ columnIndex+','+ oldValue+','+ newValue+','+row.id+','+onResponse);
	$.ajax({
		url: '<?php echo ROOT_URL ?>control/cadastros/update.php',
		type: 'POST',
		dataType: "html",
	   		data: {
			tablename : editableGrid.name,
			id: editableGrid.getRowId(rowIndex), 
                        campoid:editableGrid.getColumnName(0),
...
DatabaseGrid.prototype.fetchGrid = function(table)  {
    ...
    if(table == "aluno_atividade")
        this.editableGrid.loadJSON("<?php echo ROOT_URL ?>control/cadastros/alunos/loaddata_aluno_grid.php?db_tablename=aluno_atividade");
    if(table == "ALUNO")
        this.editableGrid.loadJSON("<?php echo ROOT_URL."control/cadastros/alunos/loaddata_aluno_grid.php?t=".$turma_filtro ?>");
};
	...
	grid.setCellRenderer("action", new CellRenderer({ 
		render: function(cell, id) {                 
		      cell.innerHTML+= "<i onclick=\"datagrid.deleteRow("+id+");\" class='fa fa-trash-o red' ></i>"
                        +" &nbsp;<i onclick=\"datagrid.abrirJanela("+id+");\" class='fa fa-pencil verde' ></i>";
		}
	})); 
...
DatabaseGrid.prototype.deleteRow = function(id) 
{
  ...
  var self = this;     
  if ( confirm('Tem certeza que você quer excluir a linha ID ' + id )  ) {
		...
		data: {
			...
			id: id ,
                        campoid: self.editableGrid.getColumnName(0)
		},
		success: function (response) 
		{ 
...
DatabaseGrid.prototype.abrirJanela = function(id) {
    window.location = '<?php echo ROOT_URL."control/cadastros/alunos/edit_aluno.php?a=" ?>'+id;
}


DatabaseGrid.prototype.addRow = function(id,table) 
{
