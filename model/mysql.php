...
/*	private $db_host    = "mysql.diretoriomogi.com.br"; // server name
	...
	*/
	...
	public function HasRecords($sql = "") {
        ...
        public function TemRegistros($sql = "") {
		if (strlen($sql) > 0) {
			$total = $this->QuerySingleValue($sql);
			if ($this->Error()) return false;
		}
                $retorno = false;
		if ($this->RowCount() > 0) {
	...
	public function UpdateRows($tableName, $valuesArray, $whereArray = null) {
        ...
        public function PriorId($tableName, $id_field, $search_value,$search_field, $ordem_field ){            
            while($search_value > 0){
                
                $sql = " SELECT count(".$id_field.") FROM ".$tableName." WHERE ".$ordem_field." < ".$search_value
                        . "  ORDER BY ".$search_field." DESC ";
                        
                if($this->TemRegistros($sql)){
                    $sql = " SELECT ".$id_field." FROM ".$tableName." WHERE ".$ordem_field." < ".$search_value
                        . "  ORDER BY ".$search_field." DESC ";
                    return $this->QuerySingleValue($sql);
                }else{
                    $search_value -= 1;
